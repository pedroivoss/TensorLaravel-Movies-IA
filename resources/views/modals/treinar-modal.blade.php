...
<script>
    // Garante que o clique no btn-treinar funciona mesmo se treinamento.js tiver erros
    document.addEventListener('DOMContentLoaded', function () {
        ...
        btnTreinar.addEventListener('click', async function () {
            if (typeof treinarModelo === 'undefined') {
                logEl.innerHTML = '';
                appendLog('⚠️ Função treinarModelo() não encontrada.');
                appendLog('Verifique se treinamento.js foi carregado corretamente.');
                return;
            }
            logEl.innerHTML = '';
            appendLog('Iniciando...');
            try {
                ...
                document.getElementById('btn-treinar-label').textContent = 'Tentar novamente';
            }
        });

        // Limpa o log ao abrir o modal novamente
        document.getElementById('modal-treinar').addEventListener('show.bs.modal', function () {
            logEl.innerHTML = 'Aguardando início...';
            btnTreinar.disabled = false;
            document.getElementById('btn-treinar-label').textContent = 'Iniciar Treinamento';
            document.getElementById('train-progress-wrap').classList.add('d-none');
            document.getElementById('train-progress-bar').style.width = '0%';
            document.getElementById('train-progress-label').textContent = '0%';
        });
    });

    // Helper global para treinamento.js atualizar a barra de progresso
    window.setTrainProgress = function (pct) {
        const wrap = document.getElementById('train-progress-wrap');
        const bar  = document.getElementById('train-progress-bar');
        const lbl  = document.getElementById('train-progress-label');
