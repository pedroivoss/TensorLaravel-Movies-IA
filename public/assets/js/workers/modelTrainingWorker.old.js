...
function encodeMovie(movie, context) {
    return tf.tidy(() => { // Otimização: Limpa tensores temporários imediatamente
        const movieGenres = typeof movie.genre === 'string'
            ? movie.genre.split(',').map(g => g.trim()).filter(Boolean)
            : movie.genre || [];
        ...
        movieGenres.forEach(genre => {
            const index = context.genreIndex[genre];
            if (index !== undefined) genreVecArray[index] = 1 * WEIGHTS.genre;
        });
        ...
        // Adicionei o rating no vetor para bater com as 'dimensions' do context
        const rating = tf.tensor1d([
            (normalize(Number(movie.rate || 0), context.minRating, context.maxRating)) * WEIGHTS.rating
        ]);

        return tf.concat([genreVec, age, rating]);
    });
}
...
function encodeUser(user, context) {
    return tf.tidy(() => {
        if (user.watchedMovies && user.watchedMovies.length) {
            // Em vez de stack (que cria matrizes 2D), vamos criar um perfil médio do usuário
            const movieVectors = user.watchedMovies.map(m => encodeMovie(m, context));
            return tf.stack(movieVectors).mean(0);
        }
        return tf.zeros([context.dimentions]);
    });
}
...
function createTrainingData(context) {
    ...
    return {
        ...
        inputDimentions: [context.dimentions * 2], // usuário + filme
    }
...
function createTrainingData(context) {
    ...
    context.users.filter(u => u.watchedMovies.length).forEach(user => {
        // Calculamos o vetor do usuário uma vez fora do loop de filmes para poupar CPU/RAM
        const userVec = encodeUser(user, context).arraySync();

        context.movies.forEach(movie => {
            const movieVec = encodeMovie(movie, context).arraySync();
            const label = user.watchedMovies.some(wm => wm.name === movie.name) ? 1 : 0;

            input.push([...userVec, ...movieVec]);
            labels.push(label);
        });
    });
    ...
    return {
        input: tf.tensor2d(input),
        labels: tf.tensor1d(labels).expandDims(1), // Ajuste para bater com a saída da rede
        inputSize: context.dimentions * 2 // Passamos apenas o NÚMERO
    };
}
...
async function configureNeuralNetAndTrain(trainData) {
    ...
    // Pegamos o tamanho real do tensor que chegou para não haver erro de mismatch
    const actualInputShape = trainData.input.shape[1];

    model.add(tf.layers.dense({
        inputShape: [actualInputShape], // Usa o shape real do dado gerado
        units: 128,
        activation: 'relu'
    }));
    ...
    await model.fit(trainData.input, trainData.labels, {
        epochs: 100,
        batchSize: 32,
        shuffle: true,
        callbacks: {
            onEpochEnd: (epoch, logs) => {
                // Verificação de segurança para o postMessage
                if (typeof workerEvents !== 'undefined') {
                    postMessage({
